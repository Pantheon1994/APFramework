...
subscribeModule.directive('subscribe', function () {
    return {
        ...
        template: '<form name="formSubscribe">' +
        ...
        '<div ng-if="subscribeIsLoading"><font color="white"><i class="fa fa-spinner fa-spin"></i></font></div><input class="form-control" name="email" type="email" placeholder="Adresse email" ng-model="user.email" required></br>' +
        '<input class="form-control" name="password" ng-minlength="minLengthPassword" placeholder="Mot de passe" type="password" ng-model="user.password" required></br>' +
        '<input class="form-control" name="passwordRepeat" ng-minlength="minLengthPassword"  placeholder="Répeter mot de passe" type="password" ng-model="user.passwordRepeat" required></br>' +
        ...
        controller: ['$scope', '$http', '$q', function ($scope, $http, $q) {
            ...
            $scope.validSubscribe = function (user) {
                ...
                $scope.newUserAccount(user).then(function (res) {
                    console.log(res);
...
subscribeModule.directive('connection', function () {
    return {
        ...
        template: '<form name="formConnection">' +
        '<div ng-if="userDoesNotExist"><div class="alert alert-danger" role="alert">{{errorNotExist}}</div></div>' +
        '<input type="email" class="form-control" name="email" ng-model="user.email" placeholder="Email" required><br/>' +
        '<input type="password" class="form-control" name="password" ng-model="user.password" placeholder="Your password" required> <br/>' +
        ...
        controller: ['$scope', '$http', '$q', '$location', function ($scope, $http, $q, $location) {
            ...
            $scope.validConnection = function (user) {
                $scope.userDoesNotExist = false;
                $scope.newConnectionAccount(user).then(function (res) {
                    console.log(res);
                    if (res.RESPONSE === "OK") {
                        $scope.user = "";
